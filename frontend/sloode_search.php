...
function sloode_search_result_func($atts)
{
  ...
  if(isset($_POST["sloode_search_key"]))
  {
   ...
   if(trim($search_key)!="" && $search_type!="")
   {
...
<div class="sloode_search_page_container">
    ...
    <input type="hidden" name="sloode_search_type" id="sloode_search_type" value="<?php echo $search_type; ?>">
    <?php
  ...
  $the_query = new WP_Query( $args_temp );
  $video_count = $the_query->post_count;
    
  $total_pages = explode('.',($video_count/$per_page));
 
  $page_count = $total_pages[0];
 ...
 ?>
  ...
  wp_reset_query();
   ...
   {
    $video_arr = get_videos_by_keyword($search_key,$per_page);
   // print_r($video_arr);
  if(!empty($video_arr))
  {
    $video_count = $video_arr[0]['TotalRecords'];
    $total_pages = explode('.',($video_count/$per_page));
    $page_count = $total_pages[0];
    if(($video_count%$per_page)!=0)
    {
 ...
 <div class="sloode_search_page_no">
  
 <?php
 if($count!=0)
 {
    echo "Page 1 of ".$page_count;
   // echo " Page: 1";
 }
 ?>
 
 </div>
 <input type="hidden" name="page_count" id="page_count" value="<?php echo $page_count; ?>">
  </div>
 ...
 <?php 
   ...
   {
	echo "<br/>No result found. Select the search type and insert one keyword at the least.<br/>";
	 echo do_shortcode("[sloode_search_form]");
   }
 ...
 query_posts($args); 
  if (have_posts()) : while (have_posts()) : the_post();
    ...
    elseif($search_type=="video")
   {
    ...
    $video_arr = get_limited_videos_by_keyword($search_key,$per_page+1,$last_post_id);
   ...
   if($count==0)
    ...
    ?>
	<div class="sloode_search_page_no"><?php
	if($count!=0)
	{
	...
	?></div>
	<input type="hidden" name="post_count" class="post_count" value="<?php echo $count; ?>">	
	</div>
<?php
die;
}
