 ...
 */
class videos_by_cat_widget extends WP_Widget {
	function __construct() {
		parent::__construct(
			'videos_by_cat', // Base ID
			__( 'Sloode-Videos by Category', 'text_domain' ), // Name
	...
	public function widget( $args, $instance ) {
		...
		if($numberposts!="" && $numberposts!=0)
		{
			...
			if($temp_data_key=="")
			{
			$video_arr = get_latest_videos_by_cat($cat_id,40);
			}
			else
			{
				 if ( false === (get_transient( $temp_data_key."_".$cat_id ) ) )
				 {
				 $video_arr = get_latest_videos_by_cat($cat_id,40);
				 $video_json = json_encode($video_arr);
				 set_transient( $temp_data_key."_".$cat_id, $video_json, 1800);
				 }
				 $video_json =  get_transient( $temp_data_key."_".$cat_id);
...
function register_videos_by_cat_widget() {
    register_widget( 'videos_by_cat_widget' );
}
add_action( 'widgets_init', 'register_videos_by_cat_widget' );
?>